...
const Navbar = ({ searchQuery, setSearchQuery }) => {
  ...
  return (
    <motion.nav 
      ...
      className="glass p-4 sm:px-6 flex flex-col md:flex-row items-center justify-between sticky top-4 sm:top-6 z-[100] mx-auto w-full gap-4 md:gap-0 border-white/10"
    >
      <div className="flex w-full md:w-auto items-center justify-between">
        {/* Brand Section */}
        <Link to="/" className="flex items-center gap-3 no-underline group scale-90 sm:scale-100 origin-left">
          <motion.div 
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mt-0.5 opacity-60 group-hover:opacity-100 transition-opacity">
              Agile Workflow
            </span>
          </div>
        </Link>

        {/* Mobile ONLY: User Dropdown snippet (logout) */}
      ...
      {/* Search Section - Stacks on Mobile */}
      <div className="w-full md:flex-1 md:max-w-md mx-0 md:mx-8 order-3 md:order-none">
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full bg-[#12131a] border border-white/5 rounded-2xl py-3 sm:py-3.5 pl-12 pr-4 text-sm text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all placeholder:text-white/20"
          />
        </div>
      </div>

      {/* Actions Section */}
      <div className="flex items-center justify-between w-full md:w-auto order-2 md:order-none px-1 md:px-0">
          ...
          {user && (
            <div className="flex items-center gap-2">
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
                onClick={handleLogout} 
                className="hidden md:flex w-11 h-11 rounded-xl bg-red-500/10 border border-red-500/20 items-center justify-center text-red-400 hover:bg-red-500/20"
              >
                <LogOut size={20} />
              </motion.button>
            </div>
